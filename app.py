...
page = st.sidebar.selectbox(
    "📄 Go to Page",
    ("Dashboard", "Nano Things", "Nano Chatbot", "Settings")
)

# Sample chatbot reply
def nano_bot(question, df):
    if df is None:
        return "Please upload your data first."
    if "apakah optimal" in question.lower():
        max_row = df.loc[df["Amount"].idxmin()]
        return f"Hasil sintesis Anda adalah {abs(max_row['Amount']):,.0f} untuk {max_row['Category']} pada {max_row['Date']}."
    return "Maaf, saya belum memahami pertanyaan Anda sepenuhnya."

# Dashboard Page
if page == "Dashboard":
    st.title("📊 NanoLab Dashboard")
...
# Chatbot Page
elif page == "Nano Chatbot":
    st.title("💬 Ask Our Finance Bot")
    st.chat_message("assistant").write("Hi! Saya adalah NanoBot. Tanyakan apapun seputar Nano!")
